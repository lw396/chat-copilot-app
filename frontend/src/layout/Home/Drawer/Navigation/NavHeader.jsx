...
function NavHeader({ search, isShow, add, open }) {
  return (
    <header className="absolute top-0 left-0 bg-white px-3">
      <div className="flex gap-2 pl-5 pt-2 w-56 ">
        <div className="flex justify-center items-center p-1 my-auto rounded-lg">
          <button onClick={open}>
            <FiAlignJustify />
          </button>
        </div>
        <div className="justify-between inline-flex px-3">
          <img loading="lazy" src={logo} className="w-[30px] h-[30px]" />
          <div className="py-1 px-1 font-semibold text-[#1c1c1c] text-sm leading-7">
            ChatCopilot
          </div>
        </div>
      </div>
